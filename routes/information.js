...
router.get('/', async function(req, res) {
  try{
    var queRes = await db.query('SELECT * FROM information');
...
router.get('/:id', async function(req, res) {
  try{
    var queRes = await db.query('SELECT * FROM information WHERE information_id=$1',
      [ req.params['id'] ]);
...
// READ specific information for a force
router.get('/force/:id', async function(req, res) {
  try{
    var queRes = await db.query('SELECT * FROM information WHERE force=$1',
      [ req.params['id'] ]);
    res.status(200).send(queRes.rows);
  }catch(err){
    console.log(err);
...
router.post('/create', async function(req, res) {
  try{
    ...
    var queRes = await db.query('INSERT INTO information(title,text,force) VALUES ($1,$2,$3) RETURNING information_id',
...
// UPDATE various field with the information
router.put('/update', async function(req, res) {
  try{
    ...
    var queRes = await db.query('SELECT fo.player_id FROM information as g LEFT JOIN forces AS fo ON (g.force=fo.force_id) WHERE g.information_id=$1',
      [ req.body['id'] ]);
...
router.delete('/delete', async function(req, res) {
  try{
    ...
    var queRes = await db.query('DELETE FROM information WHERE information_id=$1',
      [ req.body['id'] ]);
    res.sendStatus(200);
  }catch(err){
    console.log(err);
